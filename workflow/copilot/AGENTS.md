...
**Full cycle:** See **`.github/instructions/compounding-dev-cycle.instructions.md`** and **`.github/instructions/token-policy.instructions.md`** (both `applyTo: "**"`; Copilot uses them with this file and `.github/copilot-instructions.md`).
...
Apply **`.github/instructions/token-policy.instructions.md`** in every session: refine → hand off, concise but complete responses, lean diffs, internal XML blueprints for complex work.
...
2. **Always follow token-policy and the compounding dev cycle** (Plan → Code → Review/Test → Plan); see `.github/instructions/token-policy.instructions.md` and `.github/instructions/compounding-dev-cycle.instructions.md`.
...
- **`.github/instructions/token-policy.instructions.md`** — Token policy (refine → hand off; applies to all files).
