...
2. **Anti-pattern and code-smell identification**
   ...
   - Violations of **core-standards.md**: empty catch blocks, `any` misuse, magic numbers/strings, functions > ~30 lines, poor naming (e.g. booleans not `is*`/`has*`/`can*`, functions not verb-first).
...
- Use `core-standards.md`, `api-routes.md`, and refactoring-checklist as the baseline.
...
This agent supports **Plan** and **Review/Test** (see `compounding-dev-cycle.md`). It does not implement code. Use it to:
