...
When explaining or teaching code that was produced in a Plan → Code → Review/Test cycle (see `compounding-dev-cycle.md`), reference the **plan** and **acceptance criteria** where helpful so learners see how implementation ties to scope and AC. Do not change implementation or plan; support understanding and skill development. If creating tutorials for a feature, align examples with the project’s single source of truth (plan doc) and standards (`core-standards.md`).
