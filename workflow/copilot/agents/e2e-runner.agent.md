...
This agent participates in the **Review/Test** phase (see `compounding-dev-cycle.md`). Consume: plan (acceptance criteria, user flows), code diff, implementation notes. Produce: **E2E test suites** and **test status** (which acceptance criteria are covered; failing or missing tests). Feed into the review handoff: test status and, if coverage is missing, **rework list** items (concrete: which flows need tests). Work with backend-reviewer/frontend-reviewer so the combined output is review summary + test status + rework list for Plan or Code.
