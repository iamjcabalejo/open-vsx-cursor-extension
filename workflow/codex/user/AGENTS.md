...
**Discipline:** Do not expand scope beyond the plan without updating the plan first.
...
**Gates:** All acceptance criteria covered by tests; no known violations of project rules. **Code is production ready when gates pass and there are no Critical rework items.**

**Handoff rule:** If there are **Critical** rework items or gates not passed, feed back into **Plan** (rework = new AC), then **Code** (fix), then **Review/Test** again. Repeat until gates pass and no Critical issues—then code is **finalized and production ready**.
...
**Process:** Turn rework items into a short plan (scope = fixing issues, acceptance criteria = each critical item resolved, tasks for Code). Then **Code** (implement fixes) → **Review/Test** again. Repeat until no critical issues and gates pass—then declares code **production ready**.
