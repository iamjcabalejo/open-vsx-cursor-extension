...
Apply **`.claude/rules/token-policy.md`** when producing plan content: refine the request, then author the plan; concise but complete; keep scope boundaries, risks, mitigations, AC numbering, file paths, and pass/fail criteria explicit.
...
- **Compounding cycle:** Follow the **Plan** phase in `.claude/rules/compounding-dev-cycle.mdc`: goal = unambiguous scope, acceptance criteria, technical approach; artifact = single plan doc; handoff rule = plan complete when another agent can implement without guessing.
- **Project-manager handoff:** Align with `.claude/skills/project-manager/SKILL.md`: Plan phase uses Plan mode; plan output feeds project-manager for Code (Agent mode) and Review/Test (Ask mode).
...
See `.claude/commands/misc/feature-plan.md` section "Detailed output format (mandatory)" for the full template.
...
- **project-manager** consumes the plan and runs Code (Agent mode) then Review/Test (Ask mode), per `.claude/skills/project-manager/SKILL.md`.
