...
// hooks.json
{
  ...
  "hooks": {
    "afterFileEdit": [
      {"command": ".claude/hooks/format.sh", "timeout": 10}
    ]
  }
}
```
...
```json
{
  "hooks": {
    "beforeShellExecution": [
      {
        "command": ".claude/hooks/block-risky.sh",
        "timeout": 5,
...
{
  "hooks": {
    "beforeShellExecution": [{"command": ".claude/hooks/audit.sh"}],
    "afterFileEdit": [{"command": ".claude/hooks/audit.sh"}],
    "stop": [{"command": ".claude/hooks/audit.sh"}]
  }
}
```
...
```json
{
  "hooks": {
    "sessionStart": [
      {"command": ".claude/hooks/session-init.sh", "timeout": 5}
    ]
  }
}
```
...
```json
{
  "hooks": {
    "beforeShellExecution": [
      {
        "command": ".claude/hooks/approve-network.sh",
        "timeout": 30,
...
| `audit.sh` | beforeShellExecution, afterShellExecution, sessionEnd, stop | Logs events to `.claude/hooks/audit.log` |
...
This project follows **Plan → Code → Review/Test → Plan** (see `.claude/rules/compounding-dev-cycle.mdc`). Hooks are configured to support that cycle:
