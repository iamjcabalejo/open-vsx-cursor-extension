...
async function main() {
  ...
  if (!cursorResult.success) {
    ...
    assert(exists(path.join(TEMP_WORKSPACE, ".cursor-plugin")), "missing .cursor-plugin/");
    assert(exists(path.join(TEMP_WORKSPACE, ".cursor", "rules")), "missing .cursor/rules/");
    assert(exists(path.join(TEMP_WORKSPACE, ".cursor", "agents")), "missing .cursor/agents/");
    assert(exists(path.join(TEMP_WORKSPACE, ".cursor", "skills")), "missing .cursor/skills/");
  ...
  if (!claudeResult.success) {
    ...
    assert(exists(path.join(TEMP_WORKSPACE, ".claude", "agents")), "missing .claude/agents/");
    assert(exists(path.join(TEMP_WORKSPACE, ".claude", "rules")), "missing .claude/rules/");
    const claudeMd = fs.readFileSync(path.join(TEMP_WORKSPACE, "CLAUDE.md"), "utf-8");
    assert(!claudeMd.includes(".cursor/"), "CLAUDE.md should not reference .cursor/");
    console.log("   [OK] Apply Claude: .claude/ and CLAUDE.md created (no .cursor paths)");
  }
  ...
  if (!copilotResult.success) {
    ...
    assert(exists(path.join(TEMP_WORKSPACE, ".github", "copilot-instructions.md")), "missing .github/copilot-instructions.md");
    assert(exists(path.join(TEMP_WORKSPACE, ".github", "instructions")), "missing .github/instructions/");
    assert(exists(path.join(TEMP_WORKSPACE, ".github", "agents")), "missing .github/agents/");
    assert(exists(path.join(TEMP_WORKSPACE, ".github", "workflows")), "missing .github/workflows/");
  ...
  if (!cursorRemove.success) {
    ...
    const cursorGone = !exists(path.join(TEMP_WORKSPACE, ".cursor")) && !exists(path.join(TEMP_WORKSPACE, ".cursor-plugin"));
    if (cursorGone) {
      console.log("   [OK] Remove Cursor: .cursor/ and .cursor-plugin/ removed");
    } else {
      console.log("   [WARN] Remove Cursor: reported success but dirs still present (manifest may not have been set)");
    }
  }
  ...
  if (!claudeRemove.success) {
    ...
    const claudeGone = !exists(path.join(TEMP_WORKSPACE, ".claude")) && !exists(path.join(TEMP_WORKSPACE, "CLAUDE.md"));
    if (claudeGone) {
      console.log("   [OK] Remove Claude: .claude/ and CLAUDE.md removed");
    } else {
      console.log("   [WARN] Remove Claude: reported success but dirs still present");
    }
  }
  ...
  if (!copilotRemove.success) {
    ...
    if (githubGone && agentsGone) {
      ...
      console.log("   [WARN] Remove Copilot: reported success but some artifacts still present");
    }
  }
  ...
  if (exists(TEMP_WORKSPACE)) {
    try {
      ...
      console.log("\n   (Could not remove temp workspace:", TEMP_WORKSPACE, ")");
    }
  }

  console.log("\n3. Package");
  console.log("   Compile: already run (script loads from out/).");
  console.log("   vsce package: run manually. If you see 'Expected concurrency to be an integer', it is a vsce/secretlint bug;");
  console.log("   the file list showed workflow/ (171 files) included. Install VSIX in a clean profile for full checklist.");
  ...
  const codexAgents = path.join(EXTENSION_PATH, "workflow", "codex", "AGENTS.md");
  if (exists(codexSkills) && exists(codexAgents)) {
    console.log("   [OK] workflow/codex/skills/ and workflow/codex/AGENTS.md exist.");
  } else {
  ...
  console.log("Result: PASSED (Apply/Remove for Cursor, Claude, Copilot; Codex source tree).");
  console.log("Complete the checklist by running Apply/Remove in the IDE and installing the VSIX in a clean profile.");
  process.exit(0);
}
